...
dagger.#Plan & {
	...
	actions: secrets: {
		gh_token: core.#DecodeSecret & {
			input:  client.commands.gh.stdout
			format: "json"
		}
	}
	...
	actions: push: {
		for i, _ in actions.build {
			(i): docker.#Push & {
				...
				auth: {
					username: "augustfengd" // NOTE: would like to be able to reference decrypted secrets.
					secret:   actions.secrets.gh_token.output.gh.token.contents
				}
			}
		}
	}
